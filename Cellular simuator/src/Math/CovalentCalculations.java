...
public class CovalentCalculations {
	...
	public static boolean distanceIsSmallEnough(atom a, atom one, double min){
		if(currentDistanceToCovalentRatio(one,  a) < min &&
				isCurrentDistanceLessCovalent(one,a)){
			return true;
		}
		return false;
	}
	...
	public static boolean distanceIsBigEnough(atom one, atom a, double max){
		if(currentDistanceToCovalentRatio(one, a) > max &&
				isCurrentDistanceBigCovalent(one, a)){
			return true;
		}
		return false;
	}
	...
	public static double getMinimumDistance(atom one, atom thatMin){
		return (getMinimumCovalentDistance(one,thatMin))
				-Calculations.senterAvstandDouble(one, thatMin);
	}
	public static double getMaximumDistance(atom one, atom thatMax){
		return Calculations.senterAvstandDouble(one, thatMax) 
				- getMaximumCovalentDistance(one, thatMax);
	}
